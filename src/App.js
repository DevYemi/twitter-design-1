...
function App() {
  ...
  useEffect(() => {
    ...
    let angle = 360 / circleElements.length;


    for (let i = 0; i < circleElements.length; i++) {
      let rotateAngle = angle * i
      let circle = circleElements[i];
      circle.style.transform = `rotate(${rotateAngle}deg) translate(0, -200px) rotate(-${rotateAngle}deg)`;
    }
  }, []);
      ...
      <div className="CONTAINER relative overflow-hidden flex flex-col max-w-[90%] bg-white rounded-[50px] flex-1 pt-5 ">
        ...
        <section data-name="ANIMATION" className='absolute h-[78%] w-[49%] top-0 '>
          <div data-type="text" className=' absolute h-full w-full '>
            ...
            <div className='absolute left-[25%] top-[25%] text-white'>
          ...
          <div data-type='images' ref={graph} className='absolute h-[400px] w-[400px]'>
            {animatedImages.map((data, i) => (
              <div key={i} className={` w-[400px] absolute top-[50%] left-[50%]`}>
                <img src={data.url} className={`max-w-full ${data.isDecor ? 'blur-md' : ''} object-contain`} alt="img" />
              </div>
            ))}
          </div>
        </section>
